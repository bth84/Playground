    ...
    with torch.cuda.device(0):
        ...
        for epoch in range(num_epochs):
            for d_index in range(d_steps):
                ...
                ones = Variable(torch.ones([1, 1])).to(device)
                ...
                ones = Variable(torch.ones([1, 1])).to(device)
                d_fake_error = criterion(d_fake_decision, ones)  # zeros = fake
            ...
            for g_index in range(g_steps):
                ...
                ones = Variable(torch.ones([1, 1])).to(device)
                g_error = criterion(dg_fake_decision, Variable(ones))  # Train G to pretend it's genuine
                g_error.backward()
